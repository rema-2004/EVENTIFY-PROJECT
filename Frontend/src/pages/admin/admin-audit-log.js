...
(function () {
    ...
    window.initAdminAuditLog = function initAdminAuditLog() {
        ...
        $('#audit-search').addEventListener('input', (e) => { state.query = e.target.value.trim().toLowerCase(); state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-filter-user').addEventListener('change', (e) => { state.user = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-filter-type').addEventListener('change', (e) => { state.type = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-filter-role').addEventListener('change', (e) => { state.role = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-filter-status').addEventListener('change', (e) => { state.status = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-filter-location').addEventListener('change', (e) => { state.location = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        ...
        $('#audit-range-start').addEventListener('change', (e) => { state.customStart = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        $('#audit-range-end').addEventListener('change', (e) => { state.customEnd = e.target.value; state.page = 1; renderAuditLogs(); }, { signal });
        ...
        [['#audit-adv-ip', 'ip'], ['#audit-adv-device', 'device'], ['#audit-adv-browser', 'browser'], ['#audit-adv-os', 'os'], ['#audit-adv-session', 'sessionId']]
            .forEach(([sel, key]) => $(sel).addEventListener('input', (e) => { state.adv[key] = e.target.value.trim(); state.page = 1; renderAuditLogs(); }, { signal }));
        ...
        $('#audit-rows-per-page').addEventListener('change', (e) => { state.pageSize = Number(e.target.value); state.page = 1; renderAuditLogs(); }, { signal });

        $('#btn-reset-filters').addEventListener('click', resetFilters, { signal });
        $('#empty-reset-btn').addEventListener('click', resetFilters, { signal });
        $('#btn-refresh').addEventListener('click', showLoading, { signal });

        const exportMenu = $('#export-menu');
        $('#btn-export').addEventListener('click', (e) => { e.stopPropagation(); exportMenu.hidden = !exportMenu.hidden; }, { signal });
        ...
        document.addEventListener('click', (e) => { if (!e.target.closest('.row-menu')) exportMenu.hidden = true; }, { signal });

        $('#btn-details-close').addEventListener('click', closeActivityDetails, { signal });
        detailsDialog.addEventListener('click', (e) => { if (e.target === detailsDialog) closeActivityDetails(); }, { signal });
