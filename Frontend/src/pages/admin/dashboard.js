...
(function () {
    ...
    function renderUpcomingEvents(rows) {
        $('#upcoming-events').innerHTML = rows.map(row => {
            ...
            <div class="event-row">
                ...
                <a class="btn-secondary" style="padding:.45rem .9rem" href="/admin/event-review-details">View</a>
            </div>`;
        }).join('');
    }
    ...
    function wire(signal) {
        ...
        $('#dash-retry').addEventListener('click', load, { signal });
        $('#btn-dash-refresh').addEventListener('click', load, { signal });
        $('#btn-dash-export').addEventListener('click', () => toast('Dashboard export prepared for download', 'success'), { signal });
    ...
    window.initAdminDashboard = function initAdminDashboard() {
        ...
        wire(listenerController.signal);
        load();
        return () => {
            isActive = false;
