...
(function () {
            ...
            <tr>
                ...
                <td>
                    <div class="org-row-actions">
                        <a href="/org/opportunities" aria-label="Manage ${e.name}">Manage</a>
                        <a href="/org/applicants?event=${e.id}" aria-label="Applicants for ${e.name}">Applicants</a>
                    </div>
                </td>
            </tr>`).join('');
    }
    ...
    function renderRecentApplicants(list) {
        ...
        body.innerHTML = list.map((a) => `
            <tr>
                ...
                <td>
                    <div class="org-row-actions">
                        <a href="/org/applicants" aria-label="View profile of ${a.name}">View profile</a>
                        <a href="/org/applicants" class="org-action--primary" aria-label="Review application from ${a.name}">Review</a>
                    </div>
                </td>
            </tr>`).join('');
    }
    ...
    async function load(range, isActive = () => true) {
        currentRange = range;
        const overview = await window.OrgDashboardAPI.getOverview(range);
        if (!isActive()) return;
        state = overview;
    ...
    window.initOrgDashboard = function initOrgDashboard() {
        let active = true;
        const cleanups = [wireRangeFilter(), wireMobileNav()];
        const themeObserver = wireThemeRerender();
        load('30d', () => active);
